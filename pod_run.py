#!/usr/bin/env python3
# Draait op de RunPod A100: traint ronde 4 (R-Tuning) + test meteen op AA-Omniscience.
# DEEPSEEK_KEY komt via env. Adapter -> /workspace/rtune-32b-adapter
import os, json, subprocess, sys
subprocess.run([sys.executable,"-m","pip","uninstall","-y","-q","torchvision","torchaudio"])
subprocess.run([sys.executable,"-m","pip","install","-q","peft","trl","bitsandbytes","accelerate","requests","datasets"])
...
DATA="/workspace/rtune_all.jsonl"
OUT="/workspace/rtune-32b-adapter"
...
print("Trainvoorbeelden:", len(ds), flush=True)
...
# ---------- TESTEN (basis vs getraind, AA-Omniscience) ----------
ft=tr.model
def gen(q):
...
print("\n===== AA-OMNISCIENCE RONDE 4 (n="+str(N_Q)+") =====",flush=True)
