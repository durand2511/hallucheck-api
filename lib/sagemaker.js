...
// Single inference component: a custom model.py entry point loads base+adapter together at
// startup (mirrors the proven local RunPod script) -- no separate adapter inference-component,
// since djl_python's built-in HuggingFace handler couldn't be made to load Gemma4 reliably.
const INFERENCE_COMPONENT = process.env.SAGEMAKER_COMPONENT_NAME || "vankonijnenburg-custom";
...
// Single generation round against the endpoint. Runs our own custom model.py entry point
// (lib/../../../scratchpad/custom-lmi/model.py at build time) -- request/response shape is
// exactly what that script emits: {"inputs": messages, "parameters": {...}} in,
// {"generated_text": ..., "finish_reason": ...} out. No djl_python auto-detection involved.
async function invokeOnce(messages, maxNew) {
  const payload = {
    inputs: messages,
    parameters: { max_new_tokens: maxNew },
  };
  ...
  const text = (body?.generated_text ?? "").trim();
  const finishReason = body?.finish_reason ?? "length";
