...
async function requireApiKey(req) {
  ...
  const { rows } = await db.query(
    `SELECT c.id, c.email, c.stripe_customer_id, c.stripe_subscription_item_id, k.id AS api_key_id
  ...
  const r = rows[0];
  return { id: r.id, email: r.email, apiKeyId: r.api_key_id, stripeCustomerId: r.stripe_customer_id, stripeSubscriptionItemId: r.stripe_subscription_item_id };
}
