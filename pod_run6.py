#!/usr/bin/env python3
# Ronde 6: retrain op ECHT SCHONE afzien-voorbeelden (geen enkele verborgen specifieke claim).
# Test met VERS HERLADEN model (niet tr.model direct) om ronde-5's discrepantie te vermijden.
...
DATA="/workspace/rtune_clean_abstain.jsonl"
OUT="/workspace/rtune6-32b-adapter"
...
"<antwoord>\n<eerlijk eindantwoord: geef het als je het zeker weet; zeg eerlijk dat je het niet weet als dat zo is; verzin nooit feiten, noem GEEN specifieke cijfers/namen/referenties als je twijfelt>\n</antwoord>")

rows=[json.loads(l) for l in open(DATA,encoding="utf-8") if l.strip()]
ds=Dataset.from_list([{"messages":r["messages"]} for r in rows])
print("Trainvoorbeelden (clean-abstain):", len(ds), flush=True)
...
cfg=SFTConfig(output_dir=OUT,num_train_epochs=2,per_device_train_batch_size=1,gradient_accumulation_steps=16,
              learning_rate=1e-4,lr_scheduler_type="cosine",warmup_steps=10,logging_steps=10,
...
print("\n===== AA-OMNISCIENCE RONDE6 CLEAN-ABSTAIN (n="+str(N_Q)+") =====",flush=True)
